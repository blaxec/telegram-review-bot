...
async def analyze_screenshot(bot: Bot, file_id: str, task: AnalysisTask) -> Dict[str, Any]:
    ...
    prompt = ""
    # --- ИЗМЕНЕНИЕ: Полностью новые промпты для каждой задачи ---
    if task == 'google_profile_check':
        ...
        - ГЛАВНОЕ ПРАВИЛО: Пользователь может написать новый отзыв, только если его последний (самый верхний) отзыв был опубликован **3 (три) или более дней назад**. Отзывы, опубликованные год или более назад, не подходят.
