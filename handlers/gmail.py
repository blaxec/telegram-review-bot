    ...
    if current_state == UserState.GMAIL_AWAITING_VERIFICATION:
        ...
        try:
            await bot.send_message(
                user_id,
                "⏳ Время на отправку созданного аккаунта истекло. Задача отменена.<br>"
...
async def initiate_gmail_creation(callback: CallbackQuery, state: FSMContext):
    ...
    if cooldown:
        if callback.message:
            await callback.message.edit_text(
                f"Вы сможете создать следующий аккаунт через: <i>{format_timedelta(cooldown)}</i><br><br>"
    ...
    if callback.message:
        prompt_msg = await callback.message.edit_text(
            f"За создание аккаунта выдается <i>{reward_amount} звезд</i>.<br><br>"
            "Пожалуйста, укажите <i>модель вашего устройства</i> (например, iPhone 13 Pro или Samsung Galaxy S22), "
            "с которого вы будете создавать аккаунт. Эту информацию увидит администратор.<br><br>"
...
async def send_device_model_to_admin(message: Message, state: FSMContext, bot: Bot, is_another: bool):
    ...
    response_msg = await message.answer(
        f"Ваша модель устройства: <i>{device_model}</i>.<br>"
        "Запомните ее, администратор может ее уточнить.<br><br>"
    ...
    admin_notification = (
        ...
        f"отправил модель устройства для создания аккаунта Gmail:<br><br>"
        f"<i>Модель: {device_model}</i>"
    )
    if is_another:
        admin_notification += "<br><br><i>Это запрос на создание со второго устройства (пользователь на кулдауне).</i>"
...
async def send_gmail_for_verification(callback: CallbackQuery, state: FSMContext, bot: Bot, scheduler: AsyncIOScheduler):
    ...
    admin_notification = (
        f"🚨 <b>Проверка созданного Gmail аккаунта</b> 🚨<br><br>"
        f"<i>Пользователь:</i> @{callback.from_user.username} (ID: <code>{user_id}</code>)<br>"
        f"<i>Устройство:</i> <code>{device_model}</code><br><br>"
        f"<b>Данные:</b><br>"
        f"Имя: {gmail_details['name']}<br>"
        f"Фамилия: {gmail_details['surname']}<br>"
        f"Почта: {gmail_details['email']}<br>"
        f"Пароль: <code>{gmail_details['password']}</code><br><br>"
        f"<i>Инструкция для проверки:</i><br>"
        f"1. Убедитесь, что данные заполнены верно.<br>"
        f"2. Проверьте, совпадает ли модель устройства с указанной.<br>"
...
async def show_gmail_instructions(callback: CallbackQuery, state: FSMContext):
    ...
    instructions_text = (
        "<b>Инструкция по созданию аккаунта Gmail:</b><br><br>"
        "1. Откройте приложение Gmail или перейдите на сайт <code>gmail.com</code>.<br>"
        "2. Нажмите 'Создать аккаунт'.<br>"
        "3. Введите <i>Имя</i> и <i>Фамилию</i>, которые вам выдал администратор.<br>"
        "4. Придумайте и введите <i>имя пользователя</i> (адрес почты), которое вам выдали.<br>"
        "5. Введите и подтвердите <i>пароль</i>, который вам выдали.<br>"
        "6. <i>ВАЖНО:</i> Если Google просит указать номер телефона для подтверждения, пропустите этот шаг. Если пропустить нельзя, сообщите об этом в поддержку.<br>"
        "7. Завершите создание аккаунта. Резервную почту указывать не нужно.<br><br>"
...
async def back_to_gmail_verification(callback: CallbackQuery, state: FSMContext):
    ...
    user_message = (
        "✅ Администратор одобрил ваш запрос и прислал данные для создания аккаунта:<br><br>"
        "<b>Данные для создания:</b><br>"
        f"Имя: <code>{name}</code><br>"
        f"Фамилия: <code>{surname}</code><br>"
        f"Пароль: <code>{password}</code><br>"
        f"Почта: <code>{full_email}</code><br><br>"
