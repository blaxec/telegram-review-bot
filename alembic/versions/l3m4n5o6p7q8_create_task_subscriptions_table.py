...
def upgrade() -> None:
    ...
    # --- ИСПРАВЛЕНИЕ: Мы явно создаем тип с проверкой 'checkfirst=True' ---
    # Это скажет Alembic: "Создай тип, только если его еще нет".
    # Такой подход более надежен, чем create_type=False.
...
def downgrade() -> None:
    ...
    # Тип `gender_enum` здесь НЕ удаляем, так как он используется в другой миграции.
    # ### end Alembic commands ###